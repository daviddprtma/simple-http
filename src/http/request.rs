...
impl HttpRequest{
    ...
    pub fn new(request:&str)->io::Result<HttpRequest>{
        ...
        let version : Version = Version::new(request).map_err(
            |e| {
                io::Error::new(io::ErrorKind::InvalidInput,e.msg)
        ...
        let request_body : String = if let Some((_,body)) = request.split_once("\r\n\r\n"){
            body.to_string()
        }
        else{
            "".to_string()
...
impl HttpHeader{
    pub fn new(request:&str)->Option<HttpHeader>{
        let mut httpheader = HttpHeader{headers:HashMap::new()};
    ...
    fn from_str(request:&str)->Result<Self,Self::Err>{
        ...
        if let Some((method_line,_rest)) = request_split{
            ...
            for line in method_line{
                ...
                }else if line == "HTTP/2" || line == "HTTP/2"{
                    return Ok(Version::V2_0);
                };
            };
        };
        let invalid = format!("Unknown protocol version: {}",request);
